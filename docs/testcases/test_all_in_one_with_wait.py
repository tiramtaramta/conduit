...
class TestConduit(object):
    ...
    def test_login_process(self):
        ...
        assert username_check.text == user_input_data[
            0], f"Test Failed: User is not logged in ({user_input_data[0]})."
    ...
    def test_edit_settings_process(self):
        ...
        settings_check = find_element(self.driver, By.TAG_NAME, "h1")
        assert settings_check.text == "Your Settings", f"Test Failed: Page names did not match expected ({settings_check})."
        ...
        user_picture = find_element(self.driver, By.CLASS_NAME, "form-control")
        user_bio = find_element(self.driver, By.XPATH, "//textarea[@placeholder='Short bio about you']")
        ...
        update_settings = find_element(self.driver, By.XPATH, "//button[normalize-space()='Update Settings']")
        update_settings.click()

        time.sleep(2)

        # Sikeres update értesítési ablak szövegének ellenőrzése
        swal_title = find_element(self.driver, By.CLASS_NAME, "swal-title")
        assert swal_title.text == "Update successful!", "Test Failed: Settings not updated."
        ...
        img = find_element(self.driver, By.CLASS_NAME, "user-img")
        img_check = img.get_attribute("src")
        assert img_check == user_update[
            0], f"Test Failed: Image did not match expected ({user_update[0]})."
        bio = find_element(self.driver, By.CSS_SELECTOR, "div[class='user-info'] p")
        bio_check = bio.text
    ...
    def test_logout_process(self):
        ...
        sign_out = find_element(self.driver, By.XPATH, "//a[starts-with(@href, '#/login')]")
        sign_out_check = sign_out.text
    ...
    def test_delete_post_process(self):
        ...
        delete_btn = find_element(self.driver, By.XPATH, "//*[@id='app']/div/div[1]/div/div/span/button/span")
        delete_btn.click()
